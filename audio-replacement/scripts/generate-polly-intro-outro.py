"""
Generate intro/outro audio files using AWS Polly (Danielle Long-Form Neural).
Reads text from intro-outro-scripts/ and outputs MP3 files.

Usage:
    python generate-polly-intro-outro.py          # Generate all 365
    python generate-polly-intro-outro.py 0101     # Generate single day
    python generate-polly-intro-outro.py --dry-run  # Show what would be generated
"""
...
def main():
    ...
    input_dir = os.path.join(audio_replacement_dir, "intro-outro-scripts")
    output_dir = os.path.join(audio_replacement_dir, "intro-outro-audio")
    ...
    # Get list of files to process (intros only - outro is copied from single file)
    # Exception: 1231_outro.txt has a custom script
    if single_day:
        files = [f"{single_day}_intro.txt"]
        # Include outro if it's a custom one (1231)
        if single_day == "1231":
            files.append(f"{single_day}_outro.txt")
    else:
        files = sorted(f for f in os.listdir(input_dir) if f.endswith("_intro.txt"))
